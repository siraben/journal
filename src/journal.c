...
int get_magic(block * super)
{
	return super->super.magic;
}

int check_valid(block * journal)
{
	return get_magic(journal) == MAGIC ? 1 : 0;
}
...
int get_entry_count(block * journal)
{
	return journal[0].super.num_entries;
}
...
void print_entries(block * journal, int with_contents)
{
	...
	for (; i < MAX_SIZE && c <= count + 1;
	     i++, c += (found = journal[i].used)) {
		if (found) {
			printf
			    ("Entry %d: %s\n================================================================\n",
...
void print_info(block * journal)
{
	printf("Name: %s, Number of entries: %d\n",
	       journal->super.name, journal->super.num_entries);
	printf("Journal is %svalid, magic number is 0x%x\n",
	       (check_valid(journal) ? "" : "in"), get_magic(journal));
}

void change(block ** p, block * someOtherAddress)
{
	*p = someOtherAddress;
}
...
do {\
    printf(prompt);\
    if (fgets(x, sizeof(x), stdin) == 0) {exit(0);}\
    x[strlen(x) - 1] = '\0';\
} while (x[0] == '\0')
...
static int
encrypt_file(const char *target_file,
	     ...
	     key[crypto_secretstream_xchacha20poly1305_KEYBYTES])
{
	...
	do {
		...
		crypto_secretstream_xchacha20poly1305_push(&st, buf_out,
							   &out_len, buf_in,
							   rlen, NULL, 0, tag);
...
static int
decrypt_file(const char *target_file,
	     const char *source_file,
	     const unsigned char
	     key[crypto_secretstream_xchacha20poly1305_KEYBYTES])
{
	...
	do {
		...
		if (crypto_secretstream_xchacha20poly1305_pull
		    (&st, buf_out, &out_len, &tag, buf_in, rlen, NULL,
		     0) != 0) {
...
void operate(block ** journal, const char *input)
{
	...
	if (strcmp(input, "load") == 0) {
		...
		if (saved || args[0] == 'Y') {
			...
			if (access(args, F_OK) != -1) {
				if (strstr(args, "_encrypted")) {
					const char *tmp_filename = strcat
					    (strndup(args, strlen(args) - 10),
					     ".tmp");
					strncpy((char *)key,
						getpass("Password: "), 32);

					int status =
					    decrypt_file(tmp_filename, args,
							 key);
					while (status == -1) {
		...
		if (strstr(args, "_encrypted")) {
			const char *tmp_filename = strcat
			    (strndup(args, strlen(args) - 10), ".tmp");
 ask_pass:
			strncpy((char *)key, getpass("Password: "), 32);
			if (strncmp
			    ((char *)key,
			     getpass("Confirm password: "), 32) != 0) {
